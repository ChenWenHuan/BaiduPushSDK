...
int baidupush_ios_signle(char * channel_id,char * tittle,char * description)
{
	...
	if (curl){
		...
		sprintf(pTemp,"&msg_type=1");
		strcat(pForm, pTemp);
		sprintf(tmp_buf_src,"%s%sapikey=%schannel_id=%sdevice_type=4msg=%smsg_type=1timestamp=%d",HTTP_METHOD,HTTP_URL,pAppKey,channel_id,msg_buf,tick);
		strncat(tmp_buf_src,pSecretkey,strlen(pSecretkey));
		UrlEncode(tmp_buf_src,tmp_buf_urlencode,sizeof(tmp_buf_urlencode),TRUE);
		MD5_Init(&ctx);
		MD5_Update(&ctx,tmp_buf_urlencode,strlen(tmp_buf_urlencode));
...
int baidupush_ios_all(char * tittle,char * description)
{
	...
	if (curl){
		...
		sprintf(pTemp,"&msg_type=1");
		strcat(pForm, pTemp);
		sprintf(tmp_buf_src,"%s%sapikey=%sdevice_type=4msg=%smsg_type=1timestamp=%d",HTTP_METHOD,HTTP_URL,pAppKey,msg_buf,tick);
		strncat(tmp_buf_src,pSecretkey,strlen(pSecretkey));
		UrlEncode(tmp_buf_src,tmp_buf_urlencode,sizeof(tmp_buf_urlencode),TRUE);		
		MD5_Init(&ctx);
		MD5_Update(&ctx,tmp_buf_urlencode,strlen(tmp_buf_urlencode));
